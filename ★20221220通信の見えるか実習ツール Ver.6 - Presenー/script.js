...
form.myfile.addEventListener( 'change',function(e) {
    ...
    const result = e.target.files[0];
                            console.log("■■ファイルデータ　読み込み■■");
                            console.log("result 【ファイル情報を取得】:" + result + ".");
    ...
    reader.addEventListener('load',function() {
        ...
        const jsonfile = JSON.parse(reader.result);
                            console.log("jsonfile 【JavaScriptでJSONを読み込む】:" + jsonfile + ".");
...
// const datastring = JSONData["cipio.data"] ;  //オブジェクトの値を取出し配列に
                            console.log("datastring 【cipio.data文字列抽出】:" + datastring + ",");
                    
const dataArray = datastring.split( `:` ) ;  //配列の値を":"で区切りで分割
                            console.log("dataArray 【文字列':'で分割】:[" + dataArray + "]," ) ;
                    
let bitOut = dataArray[7] ;  //出力テスト
                            console.log("dataArray[7]【ﾁｪｯｸ】:" + bitOut + ",");
                    
let binOut = dataArray[10] +  dataArray[11] ;  //出力テスト
                            console.log("dataArray[10][11]【ﾁｪｯｸ】:" + binOut + ",");
                    
let ascOut = dataArray[16] + dataArray[17] + dataArray[18] + dataArray[19] + dataArray[20] ;  //出力テスト
                            console.log("dataArray[16][17][18][19][20]【ﾁｪｯｸ】:" + ascOut + ",");

console.log("＜＜ ワイヤーシャーク　データ　表出力 ＞＞");
...
function HexBinchang(inputHexValue) {
    ...
    const result = [];
                    console.log("value【16進⇒10進換算】" + value + ",");
                    console.log("resuit【初期化】: " + result + ",");
    for (let i = 0 ; i < 4 ; i++ ) {  //4回for loop  
        result.unshift(value % 2) ;  //余りを配列に先頭から
                    console.log("i【ForLoop + 1 繰返し回数】: " + i + ",");
                    console.log("resuit【value % 2 余り配列先頭格納】: " + result + ",");
        value = Math.trunc( value / 2 );   //除算結果を整数で戻す
                    console.log("value【= value / 2 除算結果整数格納】: " + value  + ",");
    }
return result;
}
...
function HexByteChange(InputHexArray) {
    ...
    for (let i = 0 ; i < 2 ; i++ ) {
        ...
        resultarray.push(value);
                    ...
                    console.log("resultarray【上位byte⇔下位byte入替】:[" + resultarray + "],");
return resultarray ;
}
...
function DForeBinary(Arrays) {
    let resuit = 0;
                console.log("resuit 【初期化】: " + resuit + ",");
    let beki = Arrays.length - 1 ;
                console.log("beki 【配列要素数 -1】:" + beki + ",");
    for ( let i = 0 ; i < Arrays.length ; i++ ) {
                console.log("i【ForLoop + 1 繰返し回数】: " + i + ",");
    let Array = Arrays[i];
                console.log("Array【Arrays[i] 配列順(上位⇒)取出し格納】:" + Array + ",");
    resuit = resuit + ( Array * ( 16 ** (beki - i )))
                console.log("resuit【resuit+(Array*(16**(beki-i))) 16進⇒10進、加算】: " + resuit  + ",");
            }
return resuit ;
}
...
btn2.addEventListener('click' , () => {
        ...
        InputHexArray2.push(parseInt( inputHex2d.value , 16 )  ) 
                        console.log("InputHexArray2【16進⇒10進 配列格納】[" + InputHexArray2 + "],")
...
const decout4 = document.getElementById('decout4') ;
                    ...
                    console.log("----  Asscii ----");  //アスキーコード　文字列表記　変換

const AsciiCodeObj = { "00" : "NUL" ,
                       "01" : "SOH" ,
                       "02" : "STX" ,
                       "03" : "ETX" ,
                       "04" : "EOT" ,
...
function AscDecimal(InputAscCodoArray) {
    const AscDecimalArray =
        InputAscCodoArray.map(Element => AsciiCodeObj[Element]);
                            console.log("AscDecimalArray 【文字列変換 => 格納】 :[" +AscDecimalArray + "],");
...
btn3.addEventListener('click' , () => {
    ...
    asciiaray3.push( inputHex3e.value )   
                    console.log("asciiaray3 【ASCIIコード => 格納】 :[" + asciiaray3 + "],")
